...
class InputExample(object):
  ...
  def __init__(self, guid, user_name, user_desc, user_loc, user_gender, tweet_topic, tweet_text, label=None):

    ## we need different fields to indicate what's missing. 
    ## need to indicate segment id which is @type_ids as well
    self.guid = guid
    self.user_name = user_name
    self.user_desc = user_desc ## self description 
    self.user_loc = user_loc ## location 
    self.user_gender = user_gender ## may just mask it if not exist
...
class QnliProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir, name):
    ...
    if name is None: 
      name = 'train.tsv'
    return self._create_examples(self._read_tsv(os.path.join(data_dir, name)), "train")

  def get_dev_examples(self, data_dir, name):
    """See base class."""
    if name is None: 
      name = 'dev.tsv'
    return self._create_examples(self._read_tsv(os.path.join(data_dir, name)), "dev_matched")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      guid = "%s-%s" % (set_type, line[0])
      examples.append(
        InputExample(guid=guid, user_name=line[1], user_desc=line[2], user_loc=line[3], user_gender=line[4], tweet_topic=line[5], tweet_text=line[6], label=line[-1]))
    return examples
