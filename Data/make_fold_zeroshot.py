...
def move_zeroshot_to_testset (topic_list,train,validate,test):
  # @train is panda df.
  ## @topic_list is topic we want to test on. 
  ...
  ## now we remove other topics from test ... only want to do zeroshot 
  ...
  ## now remove zeroshot from train 
  temp = train [ train['tweet_topic'].isin(topic_list) ] 
  ## add to test set 
  test = pd.concat([test, temp])
  ## keep only NON-zeroshot in train 
  train = train [ ~train['tweet_topic'].isin(topic_list) ] 
...
def submitJobs (main_dir, in_file, to_skip, filter_topic, topic_to_test_file, where_save, base_name) :
  ...
  for fold in [1]:
    ...
    if topic_list is not None:
      train,validate,test = move_zeroshot_to_testset (topic_list,train,validate,test)
